...
const Characters = ({ characters }) => {
  ...
  const randomCharacters = () => {
    ...
    for (let index = 0; index < 3; index++) {
      const randomStudent =
        filteredCharacters[
          getRandomIntInclusive(1, filteredCharacters.length - 1)
        ];
      threeCharacters.push(randomStudent);
      filteredCharacters = filteredCharacters.filter(
        (character) => character.house !== randomStudent.house
      );
    }
  ...
  return show ? (
    ...
    <div className="containerCharacters">
      <div className="card">
        {threeCharacters.map((item, index) => (
          <CardsCharacters
            image={item.image}
            house={item.house}
            name={item.name}
      ...
      <button onClick={randomCharacters} className="tryAgainButton">
        Tentar novamente
      </button>
    </div>
  );
